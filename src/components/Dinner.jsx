...
const Dinner = ({handleMeals}) => {
    ...
    useEffect(()=>{
        dinner.map(item=>setCount(prev=>({...prev, [item.enName]: 0})))
    },[])

    useEffect(()=>{
        handleMeals('dinner', count);
        countAllMeals();
    },[count])
    ...
    return (
        <div>
            <div style={{display: 'flex', flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'baseline'}}>
                <h3 style={{marginBottom: 0, width: '113px'}}>انتخاب شام</h3>
                <span style={{fontSize: '12px', marginRight: '15px', color: 'red'}}>{`(${allMeals} وعده شام انتخاب شده)`}</span>
            </div>
